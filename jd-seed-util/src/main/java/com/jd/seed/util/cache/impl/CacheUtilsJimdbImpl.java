...
public class CacheUtilsJimdbImpl implements CacheUtils {
	...
	public boolean set(String key, String value) {
		...
		try {
			...
			String errorMsg = "jimdb-->set 设置key失败 key: {},value: {}";
			JIMDB_LOG.error(errorMsg, key, value, ex);
		}
		return false;
	}
	...
	public <T> T lPop(String key, Class<T> clazz) {
		...
		try {
			...
			JIMDB_LOG.error("jimdb--> lPop Object key:" + key, ex);
		}
		return null;
	}
	...
	public String rPop(String key) {
		...
		try {
			data = jimdbClient.rPop(key);
		} catch (Exception ex) {
			String errorMsg = "jimdb--> rPop key:" + key;
			JIMDB_LOG.error(errorMsg, ex);
		}
		return data;
	}
	...
	public <T> T rPop(String key, Class<T> clazz) {
		...
		try {
			String objJsonStr = rPop(key);
			if (StringUtils.isNotBlank(objJsonStr)) {
				data = JSON.parseObject(objJsonStr, clazz);
			}
		} catch (Exception ex) {
			String errorMsg = "jimdb--> rPop Object key:" + key;
			JIMDB_LOG.error(errorMsg, ex);
		}
		return data;
	}
	...
	public Long decr(String key) {
		...
		try {
			...
			String errorMsg = "jimdb--> decr key:" + key;
			JIMDB_LOG.error(errorMsg, ex);
		}
		return data;
	}
	...
	public Long decrBy(String key, long step) {
		...
		try {
			...
			data = jimdbClient.decrBy(key, step);

		} catch (Exception ex) {
			String errorMsg = "jimdb--> decrBy key:" + key;
			JIMDB_LOG.error(errorMsg, ex);
		}
		return data;
	}
